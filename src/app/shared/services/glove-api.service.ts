...
export class GloveApiService {
              ...
              case "svgMain":
                if ($(element).length != 0) {
                  ...
                  if (($(element).length = 0)) {
                    ...
                    self.svgMain
                      .select(`#${glveType}_elite_x5F_logo`)
                      .attr({ opacity: 0 });
                    break;
                  }
              ...
              case "svgInside":
                if ($(element).length != 0) {
                  if (_.includes(element, "rise") && $(element).length != 0) {
                    self.svgInside
                      .select(`#${glveType}_x5F_vw2_x5F_rse`)
                      .attr({ opacity: 1 });
                    self.svgInside
                      .select(`#${glveType}_x5F_rise_x5F_logo`)
                      .attr({ opacity: 1 });
                  } else {
                    self.svgInside.select(element).attr({ opacity: 0 });
                    if ($(`#${glveType}_x5F_elite_x5F_logo`).length != 0) {
                      self.svgInside
                        .select(`#${glveType}_x5F_elite_x5F_logo`)
                        .attr({ opacity: 0 });
                    }
                  }
                } else {
  ...
  loadFbase() {
    ...
    Snap.load("assets/images/nine-positions/fbase_inside_view.svg", function (
      f
    ) {
  ...
  loadCatcherFB() {
    ...
    Snap.load(
      "assets/images/nine-positions/catcher_fastback_inside_view.svg",
      (f) => {
        this.svgInside.attr({ viewBox: "0 0 400 400" });
        const g = f.selectAll(
  ...
  loadPitcher() {
    ...
    Snap.load("assets/images/nine-positions/pitcher_open_back.svg", function (
      f
    ) {
      self.svgMain.attr({ viewBox: "0 0 400 400" });
      self.gloveCloneMainVertical.attr({ viewBox: "0 0 400 400" });
    ...
    // tslint:disable-next-line: only-arrow-functions
    Snap.load("assets/images/nine-positions/pitcher_side_view.svg", function (
      f
    ) {
      self.svgInside.attr({ viewBox: "0 0 400 400" });
      self.gloveCloneSideVertical.attr({ viewBox: "0 0 400 400" });
