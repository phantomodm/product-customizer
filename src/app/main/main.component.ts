...
export class MainComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.currentInput$
      ...
      .subscribe((res) => (this.input = res));
  }

  applyFill(obj: { id: string; domValue: string }) {
    this.gloveApi.getHexIdFromDomSelection({
      section: obj.id,
      value: obj.domValue,
    });
  }
  ...
  ngAfterViewInit(): void {
    ...
    jQuery(document).ready(() => {
      const hover$ = fromEvent(
        jQuery(".ywapo_input,.ywapo_input_container"),
        "mouseover"
      ).pipe(takeUntil(this.unsubscribe$), distinctUntilChanged());
      const containerChanged$ = hover$.pipe(
        map((element: any) => {
          let value = jQuery(element.currentTarget)
            .parent()
            .siblings()
            .text()
            .trim();
      ...
      const ywapoInput$ = fromEvent(jQuery(".ywapo_input"), "change").pipe(
        takeUntil(this.unsubscribe$),
        distinctUntilChanged()
      );
      const ywapoInputChange$ = ywapoInput$.pipe(
        map((element: any) => {
          let value = Number(jQuery(element.currentTarget).val()),
            values = [],
            valueString;
          value = value += 1;
          if (value != undefined) {
            valueString = _.toLower(
              jQuery(element.currentTarget).context[value.toString()].label
            );
          }
      ...
      const click$ = fromEvent(jQuery(".ywapo_input_container"), "click").pipe(
        takeUntil(this.unsubscribe$),
        distinctUntilChanged()
      );
      const ywapoClicked$ = click$.pipe(
        map((element: any) => {
