...
export class VerticalViewComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.gloveData
      .getGloveSliderColors()
      .pipe(take(1), takeUntil(this.unsubscribe$))
      .subscribe((res: GloveSlider[]) => {
        ...
        this.amEmbGloveSlider.max = (
          this.gloveEmbDataSlider.length - 1
        ).toString();
      });
    ...
    this.nysApi.currentLeatherType$.subscribe((res) => {
      ...
      switch (res) {
        ...
        case "kip":
          ...
          this.amGloveSlider.max = (
            this.filteredDataSlider.length - 1
          ).toString();
          break;
        default:
          break;
    ...
    this.errorNotifier = this.nysApi.notifyObservables$
    ...
    this.gloveData
      ...
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe((res) => {
        this.currentGloveContent = this.filteredGloveContent = res;
      });
  ...
  setGloveContent() {
    this.nysApi.currentGloveType$.subscribe((res: any) => {
      if (res != undefined) {
        var filter = [];
  ...
  setGlovePersonalization(event: string, formValue: string, attribute: string) {
    ...
    this.snackBar.open(
      `Personalization color ${event} selected.`,
      "DISMISS",
      this.snackbarDuration
    );
  }
  ...
  setGloveSize(event: any) {
    ...
    id.length == 2 ? (id += ".00") : id.length == 4 ? (id += "0") : null;
    this.snackBar.open(
      id + '" inch glove was selected.',
      "DISMISS",
      this.snackbarDuration
    );
    this.glove.size = `${id}`;
  ...
  setGloveType(position: {
    gloveSizeImg: string;
    gloveType: string;
    gloveShortName: string;
    value: string;
    attribute: string;
    menu: string;
    formControl: string;
    ...
    this.setGloveOptions(
      position.gloveShortName,
      position.value,
      position.attribute,
      position.menu,
  ...
  leatherSliderSelections(event: number) {
    const index = event;
    if (index != undefined) {
      this.nysApi.setGloveCanvas(
        this.nysApi.indexToValue(index, this.gloveCustomSlider)
      );
    } else {
  ...
  onGloveSliderChange(query = false) {
    let color: any = this.nysApi.indexToValue(
      +this.gloveSliderCustom.displayValue,
      this.filteredDataSlider
    );
  ...
  onEmbroiderySliderChange(query = false) {
    let color = this.nysApi.indexToValue(
      +this.gloveSliderEmbroidery.displayValue,
      this.gloveEmbDataSlider
    );

    switch (color) {
      case "Yellow Gold":
        jQuery(".embroidery-slider .mat-slider-thumb-label-text").text(
          "Y. Gold"
        );
        break;
      case "Neon Green":
  ...
  onSignatureChange(payload, query = false) {
    let color = this.nysApi.indexToValue(
      +this.gloveSliderSignature.displayValue,
      this.gloveEmbDataSlider
    );
    switch (color) {
      case "Yellow Gold":
        jQuery(".signature-slider .mat-slider-thumb-label-text").text(
          "Y. Gold"
        );
        break;
      case "Neon Green":
    ...
    if (query == false) {
      _.forEach(payload, (p) => {
        _.forEach(p, (value, key) => {
          if (value == color) {
            this.nysApi.applyHtmlInput({
              id: "pa_personalization-embroidery",
              value: p.value,
            });
            console.log(p.value);
  ...
  onSubmit() {
    const submit = jQuery(".single_add_to_cart_button")
